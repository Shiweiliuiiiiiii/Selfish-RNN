...
parser.add_argument('--save_1000epoch', type=str, default=randomhash + '_1000epoch.pt',
                    help='path to save the final model')
...
parser.add_argument('--finetuning2', type=int, default=1000,
                    help='When (which epochs) to switch to finetuning')
...
print('Args:', args)
print('Model total parameters:', args.density*total_params)
###############################################################################
# Training code
###############################################################################
...
if args.evaluate:
    ...
    model, criterion, optimizer = torch.load(args.evaluate)
    print('=> testing...')
    ...
    try:
        ...
        if args.optimizer == 'adam':
            optimizer = torch.optim.Adam(params, betas=(0, 0.999), eps=1e-9, weight_decay=args.wdecay)
            lr_scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', 0.5, patience=2, threshold=0)
    ...
    model, criterion, optimizer = torch.load(args.save)
    # Run on test data.
    test_loss = evaluate(test_data, test_batch_size)
    print('=' * 89)
